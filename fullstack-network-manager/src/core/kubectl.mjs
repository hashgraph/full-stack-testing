...
export class Kubectl extends ShellRunner {
  ...
  async getPodIP(podName) {
    try {
      ...
      throw new FullstackTestingError(`error on detecting IP for pod ${podName}`, e)
    }

    throw new FullstackTestingError(`No resource found for ${podName}`)
  }
  ...
  async getClusterIP(svcName) {
    try {
      ...
      throw new FullstackTestingError(`error on detecting cluster IP for svc ${svcName}`, e)
    }

    throw new FullstackTestingError(`No resource found for ${svcName}`)
  }
   ...
   * @param localPort port to be forwarded
   * @param remotePort port of the host machine
   * @returns {Promise<Array>} console output as an array of strings
   */
